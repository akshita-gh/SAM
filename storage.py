import pandas as pd
import datetime
import os
import pyttsx3
from tkinter import messagebox   

LOG_FOLDER = "attendance_logs"
LOG_FILE = os.path.join(LOG_FOLDER, "attendance.csv")

# Voice function
def speak(text):
    engine = pyttsx3.init()
    engine.say(text)
    engine.runAndWait()

def mark_attendance(name):
    name = name.strip()
    if name == "":
        message = "Name cannot be empty."
        speak(message)
        messagebox.showerror("Error", message)  
        return False, message

    os.makedirs(LOG_FOLDER, exist_ok=True)

    today = str(datetime.date.today())
    current_time = datetime.datetime.now().strftime("%H:%M:%S")

    if os.path.exists(LOG_FILE):
        df = pd.read_csv(LOG_FILE)
        duplicate = df[(df["Name"] == name) & (df["Date"] == today)]
        if not duplicate.empty:
            message = f"{name} is already present today."
            speak(message)
            messagebox.showwarning("Duplicate Entry", message)   # ⚠️ Popup for duplicate
            return False, message

    new_data = pd.DataFrame({
        "Name": [name],
        "Date": [today],
        "Time": [current_time]
    })

    new_data.to_csv(
        LOG_FILE,
        mode="a",
        header=not os.path.exists(LOG_FILE),
        index=False
    )

    message = f"Attendance marked successfully for {name}"
    speak(message)
    messagebox.showinfo("Success", message)   # ✅ Popup for success
    return True, message

def get_attendance_records():
    if os.path.exists(LOG_FILE):
        df = pd.read_csv(LOG_FILE)
        return df.values.tolist()
    return []

def view_attendance():
    if os.path.exists(LOG_FILE):
        df = pd.read_csv(LOG_FILE)
        print(df)
    else:
        print("No attendance records found.")
